...
export default function BottomSheet({ isOpen, onClose, pokemon }: BottomSheetProps) {
  ...
  // ─── Latest Discord message (only 1 shown at a time) ─────────────────────
  const [latestMessage, setLatestMessage] = useState<LiveMessage | null>(null);
  ...
  const getClientErrors = (): string[] => {
    ...
    if (evTotal > 510) errs.push(`EVs total > 510 (${evTotal})`);
    customData.moves.forEach((m, i) => {
      if (m && !availableMoves.includes(m)) errs.push(`Move ${i + 1} not available`);
    });
    if (customData.ability && availableAbilities.length > 0 && !availableAbilities.includes(customData.ability))
  ...
  const handleValidate = async (showModalFeedback = true): Promise<boolean> => {
    const clientErrs = getClientErrors();
    if (clientErrs.length > 0) {
      setModalContent({ title: 'Invalid settings', message: clientErrs.join('\n'), isValid: false, errors: clientErrs });
      setShowValidateModal(true);
      return false;
    }
    const form: PokemonFormData = {
      id: pokemon.id, name: pokemon.name, species: pokemon.name,
      level: customData.level, shiny: customData.shiny, alpha: customData.alpha,
      moves: customData.moves.filter(Boolean), ability: customData.ability,
      nature: customData.nature, ivs: customData.ivs, evs: customData.evs,
      item: customData.item, game: selectedGame,
    };
    const result = await Promise.resolve(validatePokemon(form));
    if (showModalFeedback || !result.isValid) {
      setModalContent({
        title: result.isValid ? (result.warnings?.length ? 'Valid (with warnings)' : 'Valid!') : 'Validation failed',
        message: result.isValid ? (result.warnings?.length ? result.warnings.join('\n') : 'Legal Pokémon ✓') : result.errors?.join('\n') || 'Unknown error',
        isValid: result.isValid, errors: result.errors, warnings: result.warnings,
      });
      setShowValidateModal(true);
    }
    return result.isValid;
  ...
  const handleTrade = async () => {
    ...
    const tradePayload = {
      pokemonName: formatPokemonName(pokemon.name),
      pokemonId: pokemon.id,
      shiny: customData.shiny, alpha: customData.alpha,
      level: customData.level, nature: customData.nature,
      ability: customData.ability,
      moves: customData.moves.filter(m => m && m.trim() !== ''),
      ivs: customData.ivs, evs: customData.evs,
      item: customData.item, ball: customData.ball,
      teraType: customData.teraType, game: selectedGame,
      ot: customData.ot, tid: customData.tid, sid: customData.sid,
      nickname: customData.nickname,
    };
    ...
    try {
      const resp = await fetch('/api/trade', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ pokemon: tradePayload, userId: user?.uid || null, userName: user?.displayName || 'Guest Trainer' }),
      });
      ...
      try {
        ...
        setModalContent({ title: 'Server Error', message: `Server ne valid response nahi diya (status ${resp.status}).`, isValid: false, errors: [`HTTP ${resp.status}`] });
        setShowValidateModal(true);
        return;
      }

      if (!resp.ok) {
        setModalContent({ title: 'Trade Failed', message: data.error || `Request failed (${resp.status})`, isValid: false, errors: [data.error || `HTTP ${resp.status}`] });
        setShowValidateModal(true);
        return;
      }
      ...
      if (data.success) {
        ...
        try {
          await saveTradeToFirestore(finalUserId, { ...tradePayload, discordMessageId: data.messageId, linkCode: data.linkCode || 'WAITING' });
        } catch (fsErr) {
        ...
        if (user?.uid) {
          addTradeToHistory?.({ userId: user.uid, pokemonId: pokemon.id, pokemonName: pokemon.name, linkCode: data.linkCode || 'PENDING', status: 'pending', game: selectedGame });
        }
      } else {
        setModalContent({ title: 'Trade Failed', message: data.error || 'Bot error', isValid: false, errors: [data.error || 'Unknown'] });
        setShowValidateModal(true);
      }
    } catch (err) {
      setModalContent({ title: 'Network Error', message: 'Connection failed. Page refresh karke phir try karein.', isValid: false, errors: ['Network error'] });
      setShowValidateModal(true);
    } finally {
      setIsLoading(false);
    }
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      try {
        const res = await fetch(
          `/api/trade/messages?game=${encodeURIComponent(game)}&discordUser=${encodeURIComponent(discordName)}`
        );
        if (!res.ok) return;
        ...
        setModalContent((prev: any) => {
          ...
          if (result.queuePosition != null) {
            next.queuePosition = result.queuePosition;
          }
    ...
    poll();
    const interval = setInterval(poll, 3500);   // 3.5 seconds - better live feel
    return () => clearInterval(interval);
  }, [isPolling, showModal, modalContent?.linkCode, modalContent?.game, selectedGame, customData.ot, user?.displayName]);

  const updateIV = (stat: keyof typeof customData.ivs, value: string) => {
    const num = Number(value);
    setCustomData(p => ({ ...p, ivs: { ...p.ivs, [stat]: Number.isNaN(num) ? 31 : Math.min(31, Math.max(0, num)) } }));
  };

  const updateEV = (stat: keyof typeof customData.evs, value: string) => {
    const num = Number(value);
    setCustomData(p => ({ ...p, evs: { ...p.evs, [stat]: Number.isNaN(num) ? 0 : Math.min(252, Math.max(0, num)) } }));
  };
  ...
  return (
    <>
      ...
      <div className={`fixed inset-0 z-50 flex items-end bg-black/60 backdrop-blur-sm transition-opacity duration-300 ${isOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
        <div className={`bg-white dark:bg-gray-900 w-full max-h-[92vh] overflow-hidden rounded-t-3xl shadow-2xl transform transition-transform duration-300 ${isOpen ? 'translate-y-0' : 'translate-y-full'}`}>
          ...
          <div className="bg-gradient-to-r from-blue-600 to-cyan-600 px-4 py-4 flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-white font-bold text-xl">{formatPokemonName(pokemon.name)}</h3>
                <div className="flex items-center gap-2 text-sm text-blue-100">
                  <span>#{String(pokemon.id).padStart(4, '0')}</span>
                  {pokemon.types?.map((t: any) => (
                    <span key={t.type.name} className="px-2 py-0.5 text-xs rounded-full bg-white/20">{t.type.name}</span>
                  ))}
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-3">
                  <div>
                    <label className="block text-sm mb-1 font-medium">Nickname</label>
                    <input maxLength={12} value={customData.nickname} onChange={e => setCustomData(p => ({ ...p, nickname: e.target.value }))} className="w-full border rounded-lg px-3 py-2 dark:bg-gray-800 dark:border-gray-700" />
                  </div>
                  <div className="grid grid-cols-3 gap-2">
                    <div><label className="block text-xs mb-1">OT</label><input value={customData.ot} onChange={e => setCustomData(p => ({ ...p, ot: e.target.value.slice(0, 12) }))} className="w-full border rounded px-2 py-1.5 text-sm dark:bg-gray-800 dark:border-gray-700" maxLength={12} /></div>
                    <div><label className="block text-xs mb-1">TID</label><input value={customData.tid} onChange={e => setCustomData(p => ({ ...p, tid: e.target.value.replace(/\D/g, '').slice(0, 5) }))} className="w-full border rounded px-2 py-1.5 text-sm dark:bg-gray-800 dark:border-gray-700" maxLength={5} placeholder="optional" /></div>
                    <div><label className="block text-xs mb-1">SID</label><input value={customData.sid} onChange={e => setCustomData(p => ({ ...p, sid: e.target.value.replace(/\D/g, '').slice(0, 5) }))} className="w-full border rounded px-2 py-1.5 text-sm dark:bg-gray-800 dark:border-gray-700" maxLength={5} placeholder="optional" /></div>
                  </div>
                  <div>
                    <label className="block text-sm mb-1 font-medium">Nature</label>
                    <select value={customData.nature} onChange={e => setCustomData(p => ({ ...p, nature: e.target.value }))} className="w-full border rounded-lg px-3 py-2 dark:bg-gray-800 dark:border-gray-700">
                      {NATURES.map(n => <option key={n} value={n}>{n}</option>)}
                    </select>
                  </div>
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <label className="block text-sm mb-1 font-medium">Level</label>
                      <input type="number" min={1} max={100} value={customData.level} onChange={e => { const v = Number(e.target.value); setCustomData(p => ({ ...p, level: Number.isNaN(v) ? 50 : Math.min(100, Math.max(1, v)) })); }} className="w-full border rounded-lg px-3 py-2 text-center dark:bg-gray-800 dark:border-gray-700" />
                    </div>
                    <div>
                      <label className="block text-sm mb-1 font-medium">Item</label>
                      <select value={customData.item} onChange={e => setCustomData(p => ({ ...p, item: e.target.value }))} className="w-full border rounded-lg px-3 py-2 dark:bg-gray-800 dark:border-gray-700">
                        {COMMON_ITEMS.map(i => <option key={i} value={i}>{i}</option>)}
                      </select>
                    </div>
                  </div>
                  <div>
                    <label className="block text-sm mb-1 font-medium flex items-center gap-2">
                      Ability <span className="text-xs text-gray-400 font-normal">(optional)</span>
                      {isLoadingData && <span className="text-xs text-gray-500">(loading…)</span>}
                    </label>
                    <select value={customData.ability} onChange={e => setCustomData(p => ({ ...p, ability: e.target.value }))} disabled={isLoadingData} className="w-full border rounded-lg px-3 py-2 dark:bg-gray-800 dark:border-gray-700 disabled:opacity-50">
                      <option value="">— Bot default —</option>
                      {availableAbilities.map(a => <option key={a} value={a}>{a}</option>)}
                    </select>
                  </div>
                  <div className="flex flex-col gap-2 pt-1">
                    <label className="flex items-center gap-2"><input type="checkbox" checked={customData.shiny} onChange={e => setCustomData(p => ({ ...p, shiny: e.target.checked }))} className="rounded text-yellow-500" /><span>Shiny</span></label>
                    <label className="flex items-center gap-2"><input type="checkbox" checked={customData.alpha} onChange={e => setCustomData(p => ({ ...p, alpha: e.target.checked }))} className="rounded text-purple-600" /><span>Alpha (Legends: Arceus)</span></label>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-4">
                <div className="flex justify-between items-center">
                  <h4 className="font-semibold text-lg border-b pb-1">Stats</h4>
                  <div className="text-sm text-gray-600 dark:text-gray-400">EVs {evTotal} / 510</div>
                </div>
                <div className="grid grid-cols-3 gap-3">
                  {['hp', 'atk', 'def', 'spa', 'spd', 'spe'].map(stat => (
                    <div key={stat} className="space-y-2">
                      <div className="text-center text-xs font-medium uppercase tracking-wide text-gray-600 dark:text-gray-400">{stat}</div>
                      <input type="number" min={0} max={31} value={customData.ivs[stat as keyof typeof customData.ivs]} onChange={e => updateIV(stat as any, e.target.value)} className="w-full text-center border rounded py-1.5 text-sm dark:bg-gray-800 dark:border-gray-700" placeholder="IV" />
                      <input type="number" min={0} max={252} step={4} value={customData.evs[stat as keyof typeof customData.evs]} onChange={e => updateEV(stat as any, e.target.value)} className="w-full text-center border rounded py-1.5 text-sm dark:bg-gray-800 dark:border-gray-700" placeholder="EV" />
                    </div>
                  ))}
                </div>
              </div>
              ...
              {/* 3. Moves + Tera + Ball */}
              <div className="space-y-4">
                <h4 className="font-semibold text-lg border-b pb-1 flex items-center gap-2">Moves {isLoadingData && <span className="text-xs text-gray-500">(loading…)</span>}</h4>
                <div className="space-y-2">
                  {[0, 1, 2, 3].map(i => (
                    <select key={i} value={customData.moves[i]} onChange={e => { const copy = [...customData.moves]; copy[i] = e.target.value; setCustomData(p => ({ ...p, moves: copy })); }} disabled={isLoadingData} className="w-full border rounded-lg px-3 py-2 dark:bg-gray-800 dark:border-gray-700 disabled:opacity-50">
                      <option value="">Move {i + 1} — empty</option>
                ...
                <div>
                  <label className="block text-sm mb-1 font-medium">Tera Type</label>
                  <select value={customData.teraType} onChange={e => setCustomData(p => ({ ...p, teraType: e.target.value }))} className="w-full border rounded-lg px-3 py-2 dark:bg-gray-800 dark:border-gray-700">
                    {TERA_TYPES.map(t => <option key={t} value={t}>{t}</option>)}
                  </select>
                </div>
                <div>
                  <label className="block text-sm mb-1 font-medium">Poké Ball</label>
                  <select value={customData.ball} onChange={e => setCustomData(p => ({ ...p, ball: e.target.value }))} className="w-full border rounded-lg px-3 py-2 dark:bg-gray-800 dark:border-gray-700">
                    {COMMON_BALLS.map(b => <option key={b} value={b}>{b}</option>)}
                  </select>
                </div>
              </div>

              {/* 4. Game + Actions + Preview */}
              <div className="space-y-5">
                <div>
                  <label className="block text-sm mb-1 font-medium">Game</label>
                  <select value={selectedGame} onChange={e => setSelectedGame(e.target.value)} className="w-full border rounded-lg px-3 py-2 dark:bg-gray-800 dark:border-gray-700">
                    <option value="scarlet-violet">Scarlet / Violet</option>
                    <option value="sword-shield">Sword / Shield</option>
                    <option value="bdsp">BDSP</option>
                    <option value="legends-arceus">Legends: Arceus</option>
                ...
                {pokemonOwned && (
                  <div className="bg-green-100 dark:bg-green-900/40 text-green-800 dark:text-green-200 px-4 py-2 rounded-lg text-center text-sm font-medium">✓ Already in your Pokédex</div>
                )}
                <div className="space-y-3">
                  <a href="https://discord.gg/blaines" target="_blank" rel="noopener noreferrer" className="block text-center py-3 px-4 bg-gradient-to-r from-indigo-600 to-purple-700 text-white rounded-lg font-semibold hover:brightness-110 transition">💬 Join Discord</a>
                  <button onClick={() => { void handleValidate(true); }} disabled={isLoading || isLoadingData} className="w-full py-3 px-4 bg-gradient-to-r from-blue-600 to-cyan-600 text-white rounded-lg font-semibold disabled:opacity-60 hover:brightness-110 transition">✅ Validate</button>
                  <button onClick={handleTrade} disabled={isLoading || isLoadingData} className="w-full py-3 px-4 bg-gradient-to-r from-green-600 to-emerald-700 text-white rounded-lg font-semibold disabled:opacity-60 hover:brightness-110 transition">{isLoading ? 'Sending…' : '🔄 Start Trade'}</button>
                </div>
                <div className="bg-gray-50 dark:bg-gray-800 rounded-lg p-3 text-sm font-mono whitespace-pre-wrap border border-gray-200 dark:border-gray-700">
                  <div className="font-semibold mb-2 text-base font-sans">Trade Command Preview</div>
                  {commandText}
                  <button onClick={() => navigator.clipboard.writeText(commandText)} className="mt-3 text-xs text-blue-600 dark:text-blue-400 hover:underline block">Copy to clipboard</button>
                </div>
              </div>
            </div>
          </div>
      ...
      {showValidateModal && modalContent && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/70 px-4">
          <div className={`rounded-2xl max-w-lg w-full overflow-hidden shadow-2xl border ${modalContent.isValid ? 'bg-gradient-to-b from-green-700 to-green-900 border-green-400/40' : 'bg-gradient-to-b from-red-700 to-red-900 border-red-400/40'}`}>
            <div className={`py-6 px-6 text-center ${modalContent.isValid ? 'bg-gradient-to-r from-green-600 to-emerald-700' : 'bg-gradient-to-r from-red-600 to-rose-700'}`}>
              <div className="text-5xl mb-3">{modalContent.isValid ? '✅' : '⚠️'}</div>
              <h2 className="text-2xl font-bold text-white">{modalContent.title}</h2>
            </div>
            <div className="p-6 space-y-5 text-center">
              <p className="whitespace-pre-line text-gray-200 text-sm leading-relaxed">{modalContent.message}</p>
              {modalContent.errors?.length > 0 && (
                <div className="bg-black/30 rounded-lg p-4 text-left text-red-200 text-sm max-h-60 overflow-y-auto">
                  {modalContent.errors.map((err: string, i: number) => <div key={i}>• {err}</div>)}
                </div>
              )}
              {modalContent.warnings?.length > 0 && (
                <div className="bg-yellow-900/40 rounded-lg p-4 text-yellow-200 text-sm">{modalContent.warnings.join('\n')}</div>
              )}
              <button onClick={() => setShowValidateModal(false)} className="w-full py-3 bg-white/20 hover:bg-white/30 text-white rounded-xl font-medium transition">Close</button>
            </div>
          </div>
        </div>
      )}

      {/* ─── Trade Modal — link code + latest bot message ─────────────────── */}
      {showModal && modalContent && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/70 px-4">
          <div className="bg-[#1a1d23] rounded-2xl max-w-md w-full overflow-hidden shadow-2xl border border-white/10">

            {/* Modal header */}
            <div className="bg-gradient-to-r from-green-600 to-emerald-700 py-5 px-6 flex items-center justify-between">
              <div className="flex items-center gap-3">
                <img
                  src={`https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/${pokemon.id}.png`}
                  alt={pokemon.name}
                  className="w-11 h-11 bg-white/20 rounded-full p-0.5"
                />
                <div>
                  <h2 className="text-base font-bold text-white">{modalContent.title}</h2>
                  <p className="text-green-200 text-xs">
                    {formatPokemonName(pokemon.name)} • Lv.{customData.level}
                    {customData.shiny ? ' ✨' : ''}
                  </p>
                </div>
              </div>
            ...
            <div className="p-4 space-y-3">
              ...
              <div className="flex gap-3">
                <div className="flex-1 bg-gray-900/80 rounded-xl p-4 text-center border border-gray-700/60">
                  <div className="text-gray-400 text-xs mb-1.5 uppercase tracking-wider">
                    Link Code
                  </div>

                  {modalContent.linkCode === 'WAITING' ? (
                    <div className="flex items-center justify-center gap-2">
                      <div className="w-4 h-4 border-2 border-green-500 border-t-transparent rounded-full animate-spin" />
                      <span className="text-gray-400 text-sm">Waiting for code...</span>
                    </div>
                  ) : (
                ...
                <div className="flex-1 bg-gray-900/80 rounded-xl p-4 text-center border border-gray-700/60">
                  ...
                  {modalContent.queuePosition != null ? (
                    <div className="text-2xl font-bold text-blue-400">
                      #{modalContent.queuePosition}
                    </div>
                  ) : (
                    <div className="flex items-center justify-center gap-2">
                      <div className="w-4 h-4 border-2 border-blue-500 border-t-transparent rounded-full animate-spin" />
                      <span className="text-gray-400 text-sm">Fetching queue...</span>
                    </div>
                  )}
                </div>
              </div>
              ...
              <div className="rounded-xl overflow-hidden border border-white/5">
                ...
                {/* Chat header */}
                <div className="flex items-center justify-between px-3 py-2 bg-[#2b2d31]">
                ...
                {/* Message body */}
                <div className="bg-[#313338] p-3 min-h-[72px] flex items-center">
                  ...
                  {!latestMessage ? (
                    ...
                    <div className="flex gap-2.5 items-start w-full">
                      ...
                      {/* Avatar */}
                      <div className="w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold text-white bg-[#5865f2]">
                        {getInitials(latestMessage.author)}
                      </div>
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-1.5 mb-0.5">
                          ...
                          {isBotName(latestMessage.author) && (
                            <span className="text-[9px] bg-[#5865f2]/30 text-[#5865f2] px-1 py-0.5 rounded font-medium">
                              BOT
                            </span>
                          )}
              ...
              {/* Success banner */}
              {modalContent.linkCode && modalContent.linkCode !== 'WAITING' && (
                <div className="bg-green-900/30 border border-green-500/30 rounded-xl p-3 flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div className="text-gray-400 text-xs">
                      {modalContent.botIGN
                        ? `Bot IGN: ${modalContent.botIGN}`
                        : 'Enter the code in your game'}
                    </div>
                  </div>
                </div>
              )}
              ...
              <button
                ...
                className="w-full py-2.5 bg-white/10 hover:bg-white/15 text-white rounded-xl font-medium transition text-sm"
              >
                Close
              </button>
